...
if __name__ == "__main__":
    ...
    parse.add_argument("-d", "--dataset", default='uai',help="dataset", type=str)  #citeseer，uai, flickr， acm， BlogCatalog， cora
    ...
    sadj, fadj, tadj = load_graph(args.labelrate, config)
    features, labels, idx_train, idx_test = load_data(config)

    model = MDRL_2v(nfeat = config.fdim,
              nhid1 = config.nhid1,
              nhid2 = config.nhid2,
    ...
    def train(model, epochs):
        model.train()
        optimizer.zero_grad()
        output, att, shared_loss, spec_loss, disttill_loss, _, _ = model(features, sadj, fadj, config.s_rec, config.sim_v)
        ...
        acc_test, macro_f1, acc_test2, acc_test3 = main_test(model)
    ...
    def eval_edge_pred(emb, sadj, fadj):
        ...
        # roc_auc_s = roc_auc_score(edge_labels_s, logits_s)
        ap_score_s = average_precision_score(edge_labels_s, logits_s)
        # roc_auc_f = roc_auc_score(edge_labels_f, logits_f)
    ...
    def main_test(model):
        model.eval()
        output, _, _, _, _, emb1, emb2 = model(features, sadj, fadj, config.s_rec, config.sim_v)


        output2 = model.MLP(emb1)
        output3 = model.MLP(emb2)

        acc_test = accuracy(output[idx_test], labels[idx_test])
        acc_test2 = accuracy(output2[idx_test], labels[idx_test])
        acc_test3 = accuracy(output3[idx_test], labels[idx_test])
        label_max = []
        for idx in idx_test:
            label_max.append(torch.argmax(output[idx]).item())
        labelcpu = labels[idx_test].data.cpu()
        macro_f1 = f1_score(labelcpu, label_max, average='macro')
        return acc_test, macro_f1, acc_test2, acc_test3
    acc_max = 0
    f1_max = 0
    epoch_max = 0
